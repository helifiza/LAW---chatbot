...
class FakeGeminiClient:
    ...
    def embed(self, model: str, texts, *, task_type: str, output_dimension: int) -> list[list[float]]:
        batch = list(texts)
...
class EmbeddingServiceTests(unittest.TestCase):
    def test_batches_and_preserves_input_order(self) -> None:
        ...
        service = EmbeddingService(client, "bge-m3", batch_size=2, output_dimension=2)
    ...
    def test_query_uses_the_same_embedding_model(self) -> None:
        client = FakeGeminiClient()
        service = EmbeddingService(client, "bge-m3", batch_size=16, output_dimension=2)
